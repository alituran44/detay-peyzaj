...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emailSubject = customTemplate?.subject || `🔑 [Detay Peyzaj] Şifre Sıfırlama Kodunuz: ${cleanCode}`;
    const heading = customTemplate?.heading || 'Şifre Sıfırlama Talebiniz Alındı';
    const badge = customTemplate?.badge || 'HESAP GÜVENLİĞİ & ŞİFRE SIFIRLAMA';
    const badgeColor = customTemplate?.badgeColor || '#ea580c';
    const footerNote = customTemplate?.footerNote || 'Güvenlik Uyarısı: Detay Peyzaj çalışanları sizden hiçbir zaman şifrenizi veya onay kodunuzu talep etmez.';
    const buttonText = customTemplate?.buttonText || 'Şifremi Sıfırla & Giriş Yap';
    ...
    let bodyFormatted = customTemplate?.bodyContent
      ? customTemplate.bodyContent
          ...
          .replace(/\n/g, '<br/>')
      : `Sayın <strong>${nameToGreet}</strong>,<br/><br/>Detay Peyzaj müşteri hesabınız için şifre sıfırlama talebinde bulundunuz. Yeni şifrenizi belirlemek için aşağıdaki 6 haneli güvenlik kodunu kullanınız:`;
    ...
    const htmlBody = `
      <div style="font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Roboto, sans-serif; max-width: 600px; margin: 0 auto; background-color: #090d16; color: #f8fafc; border-radius: 20px; overflow: hidden; border: 1px solid #ea580c; box-shadow: 0 20px 50px rgba(0,0,0,0.5);">
        <!-- Header -->
        <div style="background: linear-gradient(135deg, #ea580c 0%, #9a3412 100%); padding: 28px 20px; text-align: center;">
          <div style="display: inline-block; padding: 4px 12px; background: rgba(0,0,0,0.25); border-radius: 9999px; font-size: 11px; font-weight: 800; letter-spacing: 1.5px; color: #ffedd5; margin-bottom: 8px;">
        ...
        <div style="padding: 32px 24px; text-align: center;">
          ...
          <!-- OTP Code Box -->
          <div style="background: linear-gradient(180deg, #1e293b 0%, #0f172a 100%); border: 2px dashed #ea580c; border-radius: 16px; padding: 20px; margin: 20px 0; display: inline-block; width: 85%;">
            <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #fdba74; font-weight: 700; margin-bottom: 6px;">
              6 Haneli Güvenlik Kodunuz
            </div>
            <div style="font-size: 36px; font-weight: 900; letter-spacing: 10px; color: #ea580c; font-family: 'Courier New', monospace;">
              ${code}
            </div>
          ...
          <div style="margin: 28px 0 16px;">
            <a href="${buttonUrl}" target="_blank" style="display: inline-block; background: linear-gradient(135deg, #ea580c 0%, #c2410c 100%); color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 12px; font-weight: 700; font-size: 14px; box-shadow: 0 4px 15px rgba(234, 88, 12, 0.4);">
              ${buttonText}
            </a>
          </div>
    ...
    console.log(`[PASSWORD RESET DISPATCHED] To: ${email} | Code: ${code}`);

    await sendMailHelper({
      to: email,
      subject: emailSubject,
      html: htmlBody,
      text: `Detay Peyzaj Şifre Sıfırlama Kodunuz: ${code}`,
    });

    return res.status(200).json({
      success: true,
      email,
      message: `Şifre sıfırlama kodu ${email} adresine iletildi.`,
    });
  } catch (error) {
    console.error('Password reset error:', error);
    return res.status(500).json({ error: 'Şifre sıfırlama kodu gönderilemedi', details: String(error) });
  }
}
